...
=======
﻿import os, time
>>>>>>> f32737b (Add working Moirai-2 model (dynamic load, no register_model))
...
def _float_series(df: pd.DataFrame, col: str) -> np.ndarray:
    s = pd.to_numeric(df[col], errors="coerce").astype("float32").to_numpy()
    return s[~np.isnan(s)]

# simple metrics (depend only on numpy)
def mae(y, yhat):  return float(np.mean(np.abs(np.asarray(y)-np.asarray(yhat))))
...
def run(df: pd.DataFrame, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # tiny holdout inside the context for quick comparability to 80/20 schemes
    split = max(1, int(0.8*lookback))
    y_eval_ctx = ctx[split:]
    n_eval = min(len(y_eval_ctx), horizon)
    true_eval = y_eval_ctx[:n_eval]
    pred_eval = yhat[:n_eval]

    m_mae   = mae(true_eval, pred_eval) if n_eval>0 else float("nan")
    m_rmse  = rmse(true_eval, pred_eval) if n_eval>0 else float("nan")
    m_smape = smape_pct(true_eval, pred_eval) if n_eval>0 else float("nan")
    m_r2    = r2(true_eval, pred_eval) if n_eval>1 else float("nan")
